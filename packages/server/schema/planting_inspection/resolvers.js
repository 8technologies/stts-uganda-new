...
const plantingInspectionsResolvers = {
  ...
  Query: {
    plantingReturnInspection: async (parent, args, context) => {
      ...
      return {
        id: String(plantingReturnId),
        plantingReturnId: String(plantingReturnId),
        stages,
      };
    },
  },
  ...
  Mutation: {
    initializePlantingReturnInspection: async (parent, args, context) => {
      ...
      try {
        ...
        const core = await fetchReturnById(plantingReturnId, conn);

        if (!core) {
          throw new GraphQLError("Planting return not found");
        }

        const cropStages = await fetchCropInspectionTypes(core.cropId, conn);
        ...
        for (let i = 0; i < cropStages.length; i += 1) {
          ...
          const payload = {
            ...
            inspector_id: core.inspector.id || null,
    ...
    submitPlantingInspectionStage: async (parent, args, context) => {
      ...
      try {
        ...
        if (latestStage && inspectionTypeId === latestStage.id) {
          console.log("Matched!", status, decision, plantingReturnId);
          
          const action = await conn.execute(
            `
            UPDATE planting_returns
            SET status = ?
            WHERE id = ?
            `,
            [status, plantingReturnId]
          );

          console.log("Status updated for crop_declaration_id:", plantingReturnId);
