...
export async function importSubGrowers(sub_growers_file) {
  ...
  if (headerErrors.length > 0) {
    return {
      rows: [],
      headerErrors,
      headerErrorMessage: `Missing required columns: ${headerErrors.join(", ")}`,
    };
  }
  ...
  for (let i = 0; i < filteredRows.length; i++) {
    ...
    const expectedHarvestRaw = getRawByAliases(
      raw,
      HEADER_ALIASES.expected_harvest,
    );
    const expectedHarvest = parseExcelDate(expectedHarvestRaw);
    ...
    if (cropRaw) {
      ...
      if (m) {
        ...
        console.log(
          `[Row ${rowNum}] Resolving crop "${cropName}" and variety "${varietyName}" from DB`,
        );

        const [cropRows] = await db.execute(
          "SELECT id FROM crops WHERE LOWER(name) = LOWER(?)",
          [cropName],
        );
        const [varietyRows] = await db.execute(
          "SELECT id FROM crop_varieties WHERE LOWER(name) = LOWER(?)",
          [varietyName],
        );
        if (cropRows.length === 0)
          errors.push(`Crop "${cropName}" not found in the system`);
        if (varietyRows.length === 0)
          errors.push(`Variety "${varietyName}" not found in the system`);
    ...
    const seedClass =
      SEED_CLASS_MAP[seedClassRaw.toLowerCase()] ?? seedClassRaw;
    ...
    parsedRows.push({
      ...
      quantity_planted: parseNumber(qtyPlanted),
      expected_yield: parseNumber(expectedYield),
      phone_number: phoneNumber,
      gps_latitude: parseNumber(gpsLat),
      gps_longitude: parseNumber(gpsLng),
