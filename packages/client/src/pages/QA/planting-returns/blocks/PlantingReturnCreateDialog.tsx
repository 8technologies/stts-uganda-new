...
import { LOAD_CROPS, LOAD_CROP, GETDISTRICTS } from "@/gql/queries";
...
const PlantingReturnCreateDialog = ({
  ...
  const {data:districts, loading:districtLoading} = useQuery(GETDISTRICTS);
  const districtsData = districts?.getDistricts

  const subcountiesData = useMemo(() => {
  if (!values.district || !districtsData) return [];
  const found = districtsData.find((d: any) => d.name === values.district);
  return found?.subcounties || [];
}, [values.district, districtsData]);
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <select className="flex h-10 w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none" value={values.district || ""}
                  onChange={(e) => {
                      handleChange("district", e.target.value)
                      handleChange("subcounty", "");
                      }}>
                  <option value="">Select district</option>
                    {districtLoading && <option>Loading districts...</option>}
              ...
              <div>
                ...
                >
                  ...
                  {subcountiesData.map((subcounty:any) => (
