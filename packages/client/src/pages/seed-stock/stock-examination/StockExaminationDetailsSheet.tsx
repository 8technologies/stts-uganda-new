...
const StockExaminationDetailsSheet: React.FC<Props> = ({
  open,
  onOpenChange,
  onInspectionOpen,
  data,
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent
        side="right"
        className="w-full sm:max-w-[800px] h-full flex flex-col bg-gray-50"
      >
        {/* Header */}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-xl border p-5 shadow-sm">
              <div className="flex items-center justify-between gap-4 flex-wrap">
                <div className="flex items-center gap-4">
                  <div
                    className={`flex items-center justify-center w-14 h-14 ${categoryInfo.bg} rounded-xl`}
                  >
                    <KeenIcon
                      icon={categoryInfo.icon}
                      className={`${categoryInfo.color} text-xl`}
                    />
                  </div>
                  <div>
                ...
                {d.status && (
                  <div
                    className={`inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-medium border ${statusInfo.bg} ${statusInfo.color}`}
                  >
                    <span
            ...
            <div className="bg-white rounded-xl border p-5 shadow-sm">
              <SectionHeader
                ...
                subtitle="Basic examination details"
              />
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <DetailRow
                  icon="calendar"
                  label="Created On"
                  value={fmtDate(d.created_at)}
                />
                <DetailRow
                  icon="user"
                  label="Created By"
            ...
            {report && Object.keys(report).length > 0 && (
              <div className="bg-white rounded-xl border p-5 shadow-sm">
                <SectionHeader
                  ...
                  subtitle="Detailed inspection results"
                />
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  <DetailRow
                    icon="map"
                  ...
                  <DetailRow
                    ...
                    value={report.moisture_content}
                  />
                  <DetailRow
                    icon="bug"
                    label="Insect Damage"
                    value={report.insect_damage}
                  />
                  <DetailRow
                    icon="alert"
                    label="Moldiness"
                    value={report.moldiness}
                  />
                  <DetailRow icon="leaf" label="Weeds" value={report.weeds} />
                  <DetailRow
                    icon="sprout"
                    label="Germination (%)"
                    value={report.germination}
                  />
                </div>
              </div>
            )}
            ...
            {d.remarks && (
              <div className="bg-white rounded-xl border p-5 shadow-sm">
                <SectionHeader
                  icon="note"
                  title="Remarks"
                  subtitle="Additional notes or comments"
                />
          ...
          <div className="flex items-center gap-3">
            {d.reportUrl && (
              <a href={d.reportUrl} target="_blank" rel="noreferrer">
                <Button
                  variant="outline"
                  className="gap-2 hover:bg-green-50 text-green-700 border-green-300"
                >
