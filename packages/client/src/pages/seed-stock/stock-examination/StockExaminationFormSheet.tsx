...
import { LOAD_CROP_DECLARATIONS, LOAD_IMPORT_PERMITS, LOAD_PLANTING_RETURNS, LOAD_IMPORT_PERMIT, LOAD_STOCK_EXAMINATIONS } from '@/gql/queries';
...
const StockExaminationFormSheet: React.FC<Props> = ({ open, onOpenChange, mode, initialData, onSaved }) => {
  const [seedType, setSeedType] = useState<SeedType>('Grower_seed');
  const [motherLot, setMotherLot] = useState('');
  const [remarks, setRemarks] = useState('');
  const [selectedImportPermit, setSelectedImportPermit] = useState<string | null>(null);
  const [selectedVarietyImported, setSelectedVarietyImported] = useState<string | null>(null);
  const [selectedApprovedField, setSelectedApprovedField] = useState<string | null>(null);
  const [selectedQdsDeclaration, setSelectedQdsDeclaration] = useState<string | null>(null);
  const [selectedVarietyQds, setSelectedVarietyQds] = useState<string | null>(null);
  const [saving, setSaving] = useState(false);
    
    
  const LIST_VARS = { filter: {}} as const;
  const { data:cropDeclarations, loading, error, refetch } = useQuery(LOAD_CROP_DECLARATIONS, {
    variables: LIST_VARS
  });

  const RETURN_VARS = { filter: {status:'approved'}} as const;
  const { data:returns, loading:returnLoading, error:returnError } = useQuery(LOAD_PLANTING_RETURNS, {
    variables: RETURN_VARS
  });

  const PERMIT_VARS = {filter: { type: 'import' as const },} as const;
  const {data: listData, loading: listLoading, error: listError,} = useQuery(LOAD_IMPORT_PERMITS, { 
    variables: PERMIT_VARS 
  });

  const mockQdsDeclarations = useMemo(() => (cropDeclarations?.cropDeclarations?.items ?? []) as any[], [cropDeclarations]);
  const mockApprovedFields = useMemo(() => (returns?.plantingReturns?.items ?? []) as any[], [returns]);
  
  const mockImportPermits = useMemo(() => (listData?.importPermits?.items ?? []) as any[],[listData]);
  ...
  const currentQdsDeclaration = useMemo(
    () => mockQdsDeclarations.find((d) => String(d.id) === String(selectedQdsDeclaration)) ?? null,
  ...
  const qdsVarieties = useMemo(() => {
    ...
    if (Array.isArray(currentQdsDeclaration.crops)) return currentQdsDeclaration.crops;
    if (Array.isArray(currentQdsDeclaration.varieties)) return currentQdsDeclaration.varieties;
    return [];
  ...
  const currentImportPermit = useMemo(
    () => mockImportPermits.find((p) => String(p.id) === String(selectedImportPermit)) ?? null,
    [mockImportPermits, selectedImportPermit]
  );

  // Fetch import permit details when one is selected
  const { data: importPermitData, loading: importPermitLoading } = useQuery(LOAD_IMPORT_PERMIT, {
...
//   const mockVarieties = cropDeclarations?.varieties?.items
  ...
  // Add mutation hook
  const [saveStockExamination, { loading: savingMutation }] = useMutation(CREATE_STOCKEXAMINATION, {
  ...
  const handleSave = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.saveStockExamination?.success) {
        toast.success(data.saveStockExamination.message || 'Stock examination saved successfully');
        onSaved?.(data.saveStockExamination.data);
        onOpenChange(false);
      } else {
        toast.error(data?.saveStockExamination?.message || 'Failed to save stock examination');
      }
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent side="right" className="w-full sm:max-w-[720px] h-full flex flex-col bg-gray-50">
          ...
          <SheetTitle className="flex items-center gap-3 text-xl">
            ...
            <div>
              <div className="font-semibold">
                {mode === 'create' ? 'Create Stock Examination' : 'Edit Stock Examination'}
              </div>
              <div className="text-sm text-gray-500 font-normal mt-0.5">
                {mode === 'create' ? 'Add a new stock examination record' : 'Update examination details'}
              </div>
            </div>
          </SheetTitle>
        </SheetHeader>
        ...
        <form onSubmit={handleSave} className="flex-1 overflow-y-auto">
          <div className="p-6 space-y-6">
            ...
            <div className="bg-white rounded-lg border p-5 shadow-sm">
              ...
              <p className="text-sm text-gray-600 mb-4">Select the type of seed being examined</p>
              ...
              <div role="radiogroup" aria-label="examination category" className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                {[
                  { type: 'Import_seed', icon: 'package', label: 'Imported', desc: 'Foreign seed', color: 'from-blue-500 to-blue-600' },
                  { type: 'Grower_seed', icon: 'field', label: 'Grower', desc: 'Local grower', color: 'from-green-500 to-green-600' },
                  { type: 'QDS_seed', icon: 'file-text', label: 'QDS', desc: 'Quality declared', color: 'from-amber-500 to-amber-600' },
                ].map((btn) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="flex-1 text-left">
                        ...
                        <div className={`text-xs ${isActive ? 'text-white/80' : 'text-gray-500'} mt-0.5`}>
                          {btn.desc}
                        </div>
                      </div>
                      {isActive && (
            ...
            <div className="bg-white rounded-lg border p-5 shadow-sm">
              ...
              <div className="space-y-5">
                  ...
                  <>
                    <div>
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          {mockImportPermits.map((p) => (
                            <option key={p.id} value={p.id}>{p.countryOfOrigin || p.permitNumber || p.label}</option>
                          ))}
                        </select>
                      ...
                      <p className="text-xs text-gray-500 mt-1.5 flex items-start gap-1">
                        ...
                        <span>Select the active import permit for this examination</span>
                      </p>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          onChange={(e) => setSelectedVarietyImported(e.target.value || null)}
                          disabled={importPermitLoading}
                          required
                        >
                          <option value="">
                            {importPermitLoading ? 'Loading varieties...' : 'Choose variety'}
                          </option>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          onChange={(e) => setSelectedApprovedField(e.target.value || null)}
                          required
                        >
                      ...
                      <div className="relative">
                        ...
                        >
                          ...
                          {mockQdsDeclarations.map((q) => (
                            <option key={q.id} value={q.id}>{q.source_of_seed}</option>
                          ))}
                        </select>
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 pointer-events-none">
                          <KeenIcon icon="chevron-down" />
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <select
                          ...
                          onChange={(e) => setSelectedVarietyQds(e.target.value || null)}
                          required
                        >
                          <option value="">Choose variety</option>
                          {qdsVarieties.map((v: any) => {
                            const id = v.variety_id ?? v.varietyId ?? v.id ?? v.value;
                            const label = v.variety_name ?? v.varietyName ?? v.name ?? v.label ?? String(id);
                            return (
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-500 mt-1.5">Optional: Include any relevant comments</p>
                </div>
              </div>
            </div>
          </div>
