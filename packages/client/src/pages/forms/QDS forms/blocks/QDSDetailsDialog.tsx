    ...
    if (action === "approve") {
      try {
        const res = await approveForm({
          variables: {
            payload: { form_id: String(d?.id ?? ""), form_type: "qds" },
          },
        });
      ...
      >
        ...
        <div className="flex-1 min-h-0 overflow-y-auto px-2 pb-2 space-y-6">
          ...
          <div className="space-y-4">
            ...
            <LabeledRow label="Cropping histroy">
              {d.cropping_history || "-"}
            </LabeledRow>
            ...
            {d.have_adequate_isolation == true && (
              <LabeledRow label="Isolation distance">
                {d.isolation_distance || "-"}
              </LabeledRow>
            )}
